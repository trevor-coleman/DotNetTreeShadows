...
const TurnActionButtons = () => {
  ...
  const {currentTurn, turnOrder, currentActionType, playerBoards, tilesActiveThisTurn, undoActions} = useTypedSelector(
      state => state.game)
  ...
  const canDoAction = (actionType: GameActionType) => {
    ...
    switch (actionType) {
      ...
      case GameActionType.Plant:
        return playerBoard.pieces[PieceType[PieceType.Seed]].available > 0 &&
               light > 0;
      case GameActionType.Grow:
        ...
        treeTiles.forEach(treeTile => {
          ...
          if (largerPieces.available > 0 && light >= treeTilePiece + 1 &&
      ...
      case GameActionType.Collect:
        if (!treeTiles) return false;
        treeTiles.forEach(treeTile => {
          if (Tile.GetPieceHeight(tiles[treeTile]) != 3 || light < 4 ||
              tilesActiveThisTurn.indexOf(treeTile) > 0) return;
          result = true;
        })
        return result;
  ...
  const GameActionButton = ({actionType}: { actionType: GameActionType }) => (
      <Grid item>
        <Button disabled={isDisabled(actionType)}
                color={currentActionType == actionType
                       ? "secondary"
                       : "primary"}
  ...
  const turnActions = [
    GameActionType.Buy,
    GameActionType.Plant,
    GameActionType.Grow,
    GameActionType.Collect
  ]

  return (
      <div className={classes.root}>
        <Typography variant={'subtitle1'}
                    className={classes.title}>Actions</Typography>
        <Grid container spacing={0}>
          <Grid item container xs={8} spacing={1}>
            {turnActions.map(at => <GameActionButton key={at +
                                                          new Date().toString()}
                                                     actionType={at} />)}
          </Grid>
          <Grid item container xs={4} spacing={1} direction={"column"}><Grid
              item><Button disabled={!isPlayersTurn}
                           className={classes.endTurnButton}
                           color={light == 0
                                  ? "secondary"
                                  : undefined}
                           variant={"contained"}
                           onClick={() => gameActions.endTurn()}>End
                                                                 Turn</Button>
          </Grid>
            <Grid item>
              <Button disabled={!isPlayersTurn || undoActions.length == 0}
                      className={classes.endTurnButton}
                      color={'primary'}
